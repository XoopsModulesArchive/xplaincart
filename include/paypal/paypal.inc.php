...
$paypal['business'] = $shopConfig['email'];
$paypal['site_url'] = XOOPS_URL . '/modules/xplaincart/';
$paypal['image_url'] = '';
$paypal['success_url'] = 'success.php';	//will add with site_url later
$paypal['cancel_url'] = 'error.php';		//will add with site_url later
$paypal['notify_url'] = 'include/paypal/ipn.php';	//will add with site_url later
$paypal['return_method'] = '2'; //1=GET 2=POST               --> Use post since we will need the return values to check if order is valid
$paypal['currency_code'] = $shopConfig['currency_code']; //['USD,GBP,JPY,CAD,EUR']
$paypal['lc'] = 'US';

//$paypal['url'] = "https://www.paypal.com/cgi-bin/webscr";	//need to replace below in real situation
//$paypal['url']           = "https://www.sandbox.paypal.com/cgi-bin/webscr";
if (isset($xoopsModuleConfig['Paypal'])) {
    $paypal['url'] = $xoopsModuleConfig['Paypal'];
} else {
    $paypal['url'] = 'https://www.sandbox.paypal.com/cgi-bin/webscr';
}
...
function fsockPost($url, $data)
{
    ...
    if (!$fp) {
        ...
        fwrite($fp, 'Content-length: ' . mb_strlen($postData) . "\r\n");
