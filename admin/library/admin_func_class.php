<?php
if (file_exists('../library/define.php')) {
    require_once dirname(__DIR__) . '/library/define.php';
} else {
    require_once dirname(__DIR__, 2) . '/library/define.php';
}
if (file_exists('../library/database_class.php')) {
    require_once dirname(__DIR__) . '/library/database_class.php';
} else {
    require_once dirname(__DIR__, 2) . '/library/database_class.php';
}
class admin_func_class
{
    /*
    ...
    */
    ...
    function buildCategoryOptions($catId = 0)
    {
        $sql = "SELECT cat_id, cat_parent_id, cat_name
                FROM ".PREFIX."category
                ORDER BY cat_id";
        $result = dbQuery($sql) || die('Cannot get Product. ' . $GLOBALS['xoopsDB']->error());
    ...
    */
    ...
    {
        $tmpSrc = pathinfo(mb_strtolower($srcFile));

        $tmpDest = pathinfo(mb_strtolower($destFile));

        $size = getimagesize($srcFile);

        if ('gif' == $tmpDest['extension'] || 'jpg' == $tmpDest['extension']) {
            $destFile = substr_replace($destFile, 'jpg', -3);

            $dest = imagecreatetruecolor($w, $h);

            imageantialias($dest, true);
        } elseif ('png' == $tmpDest['extension']) {
            $dest = imagecreatetruecolor($w, $h);
